...
function UnBotCloseAll()
	...
	NPCFrame:Hide();
	UnBotFrame:Hide();
	DisplayInfomation("Close the robot action bar and enter /unbot in the chat bar to reopen the action bar.");
end
...
function InitializeUnBotFrame()
	...
	InitializeStrategy();
	DisplayInfomation("The robot controller has been initialized");
end

local function GetCommandTypeTextByType(typeIndex)
	if (typeIndex == 4) then
		return "Target the entire team";
	elseif (typeIndex == 3) then
		return "Need to select enemy target";
	elseif (typeIndex == 2) then
		return "Need to select friendly target";
	else
...
function CommandButton_OnEnter(self,index,btnType)
	...
	if (self.groupIndex > 0) then
		GameTooltip:AddDoubleLine("Excuting an order：",UnBotExecuteCommand[index],0,0.85,0.85,0,0.85,0.85);
	end
	...
	if (self.groupIndex > 0) then
		...
		GameTooltip:AddLine("Right mouse click: Close the main action bar",0.65,0.55,0,1);
	end
...
function SubCommandButton_OnLeftClick(index)
	...
	if (UnBotCommandType[index] == nil) then
		DisplayInfomation("could not find it "..tostring(index).." order.");
		return;
	end
