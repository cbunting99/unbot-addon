...
function NPCCommand_OnEnter(self,tipType,title,text,command)
	...
	elseif (tipType == 2) then
		...
		GameTooltip:AddLine("Before using the command, you need to select yourself or an NPC robot.。",1,0,0,1);
	end
	-- GameTooltip:AddLine("战斗状态下无法使用该命令。",1,1,1,1);
	-- GameTooltip:AddLine(" ",1,1,1,1);
	if (command ~= nil) then
		GameTooltip:AddDoubleLine("Excuting an order：",command,0,0.85,0.85,0,0.85,0.85);
	end
	GameTooltip:Show();
end
